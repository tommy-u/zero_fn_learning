...
int main(int argc, char* argv[]) {
  ...
  struct fann_train_data *subset_first = fann_subset_train_data(data, 0, atoi(argv[3]));

  while(1){
    fann_train_epoch_irpropm(ann, subset_first);
    fann_test_data(ann, subset_first);
  ...
  fann_save_train(subset_first, "subset.train");
  fann_destroy_train(subset_first);
